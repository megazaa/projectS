...
@onready var navigation_region_3d: NavigationRegion3D = $"../MapNav"
...
enum TimerState { WAIT_TO_ATTACK, ATTACK }
var timer_state: TimerState = TimerState.WAIT_TO_ATTACK
...
@export var path:enemy_manager
func _ready() -> void:
	add_to_group("enemy")
	for x in get_parent().get_node(path.node_name).get_children():
...
func _physics_process(delta: float) -> void:
	...
	if global_basis and Global.player_current_basis:
		label_3d.text = "enemy_basis: " + str(transform.basis.x) + " player_basis: " + str(Global.player_current_basis.x)
		angle_rag()
		#player_seen()
	if Input.is_action_just_pressed("ui_right"):
		basis.x = Vector3(1,0,0)
	if Input.is_action_just_pressed("ui_left"):
		basis = Global.player_current_basis
	if global_basis and Global.player_current_basis:
		label_3d.text = "enemy_basis: " + str(transform.basis.x) + " player_basis: " + str(Global.player_current_basis.x)
	else:
		label_3d.text = "Error: Basis not initialized!"
	move_stair(delta)
...
func is_roaming(delta):
	...
	nav_agent.target_position = all_point[next_point]
	facing_dir = (nav_agent.get_next_path_position()- global_position ).normalized() -transform.basis.z 
	#velocity = velocity.lerp(facing_dir*SPEED*delta,1.0)
	velocity = velocity.move_toward(facing_dir,delta)
	facing_dir.y = 0
...
func is_pursuiting(delta):
	...
	#velocity.y = 0  # Restrict to XZ plane
	var enemy_pos = global_position
	var next_pos = nav_agent.get_next_path_position()
	#var change_dir = (next_pos - enemy_pos).normalized()
	var change_dir = (Global.player_current_pos - enemy_pos).normalized()
	change_dir.y = 0  # Restrict target direction to XZ plane
	var velocity_new = change_dir*SPEED*delta
	#velocity = velocity_new
	velocity = velocity.move_toward(velocity_new,delta)
	velocity.y = vertical_velocity  # Restore the vertical component
...
func get_walk_sound(current_step):
	...
	if is_on_floor():
		...
		get_foot_sound.unit_size = randf_range(1,1.5)
		get_foot_sound.play()
		if current_step >= 7:
			sound_num = 1
...
func move_stair(delta):
	...
	step_cast_3d.force_shapecast_update()
			...
			#get_collider =  step_cast_3d.get_collider(0) && get_collider.is_in_group("stair")
	if step_cast_3d.is_colliding() && velocity.y <= 0.0 && !result:
...
#func player_seen():
	#var vector3minus =  transform.basis.x - Global.player_current_basis.x
	#if vector3minus.x - vector3minus.z == 0:
		#print("back")
	#if vector3minus.x - vector3minus.z >= 2:
		#print("front")
	#if transform.basis.x.x == 0 and Global.player_current_basis.x.z == transform.basis.x.z:
		#print("Right")
	#if transform.basis.x.x == 1 or transform.basis.x.x == -1 and Global.player_current_basis.x.x == transform.basis.x.z:
		#print("left")
@onready var animated_sprite_3d: AnimatedSprite3D = $AnimatedSprite3D

func angle_rag():
	var global_x = transform.basis.x
	var current_x = Global.player_current_basis.x
	var angle = acos(global_x.dot(current_x) / (global_x.length() * current_x.length()))
	var cross_product = global_x.cross(current_x)
	...
	elif angle_deg >= 22.5 && angle_deg < 67.5 :
		animated_sprite_3d.play("side-135")
	elif angle_deg >= 67.5 && angle_deg < 112.5 :
		animated_sprite_3d.play("side")
		animated_sprite_3d.flip_h = true
	elif angle_deg >= 112.5 && angle_deg < 157.5 :
		animated_sprite_3d.play("side-45")
	elif (angle_deg >= 157.5 && angle_deg <= 180) or (angle_deg >= -180 && angle_deg < -157.5) :
		animated_sprite_3d.play("front")
	elif angle_deg >= -157.5 && angle_deg < -112.5 :
		animated_sprite_3d.play("side-45")
	elif angle_deg >= -112.5 && angle_deg < -67.5 :
		animated_sprite_3d.flip_h = false
		animated_sprite_3d.play("side")
	elif angle_deg >= -67.5 && angle_deg < -22.5 :
		#animated_sprite_3d.flip_h = true
		animated_sprite_3d.play("side-135")
@onready var audio_stream_player_3d_2: AudioStreamPlayer3D = $player_sound_manager/AudioStreamPlayer3D2
@onready var action_timer: Timer = $action_timer

func check_rang_ack():
	if Global.player_hp <= 0:
		audio_stream_player_3d_2.stream = load("res://assets/sound/attack_sound/sword-stab-push-melee-weapon-236206.mp3")
		audio_stream_player_3d_2.pitch_scale = randf_range(.8,1.2)
		audio_stream_player_3d_2.play()
	if Global.player_current_pos == null:
		print("player_current_pos is null!")
		return
	if global_position.distance_to(Global.player_current_pos) <= 1.75 and in_pursuit:
		if not action_timer.is_stopped():
			return
		# Start the timer to wait for attack
		timer_state = TimerState.WAIT_TO_ATTACK
		action_timer.start(1.0)
func _on_action_timer_timeout() -> void:
	match timer_state:
			TimerState.WAIT_TO_ATTACK:
				# After waiting, attack if enemies are still present
				if global_position.distance_to(Global.player_current_pos) <= 1.75:
					Global.player_hp -= 4
					audio_stream_player_3d_2.stream = load("res://assets/sound/attack_sound/sword-stab-push-melee-weapon-236206.mp3")
					audio_stream_player_3d_2.pitch_scale = randf_range(.8,1.2)
					audio_stream_player_3d_2.play()
					# Set timer for cooldown after attack
					timer_state = TimerState.ATTACK
					action_timer.start(1.0)
				else:
					print("No enemies to attack.")
			TimerState.ATTACK:
				# After cooldown, recheck for enemies
				if global_position.distance_to(Global.player_current_pos) <= 1.75:
					print("Waiting to attack again.")
					timer_state = TimerState.WAIT_TO_ATTACK
					action_timer.start(1.0)
				else:
					print("No enemies left in the area.")
@onready var door_checker: RayCast3D = $door_checker
func door_check():
	var door = door_checker.get_collider()
	if door == null:
		return
	if door.is_in_group("interactable"):
		print("open>>>")
		door.get_parent().door_toggle()
		await get_tree().create_timer(1.0).timeout
		door.get_parent().door_toggle()
@export var possible_items = [
	{"scene": "res://scenes/drops/evidence_1.tscn", "probability": 10/3},
	{"scene": "res://scenes/drops/evidence_2.tscn", "probability": 10/3},
	{"scene": "res://scenes/drops/evidence_3.tscn", "probability": 10/3}
	]
func drop_item():
	var rand = randf()
	var cumulative = 0.0
